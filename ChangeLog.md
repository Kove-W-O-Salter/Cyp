...
* Fix copyright header indentation in `src/Data/Cyp.hs`
* Fix copyright header indentation in `src/Text/Cyp.hs`
...
to support generalized parsing streams (e.i. streams other than `String`).
This allows us to write `Parser`s that parse the output of a tokeniser using
...
Cons:
* You can parse the output of tokenisers
* You a more customizable way of chaining `Parser`s
...
`(|>)`, `(/>)` and `convert`. Secondly, instead of using the
`Alternative` combinator `(<|>)` you use `($>)`. Thirdly the combinators
`some` and `many` have moved to `Text.Cyp.Combinators`. Fourthly,
instead of passing one parameter to the `Parser` type, you pass
second which specifies the type of the elements of the stream.
Fifthly, a `Parser` is now a function of the type:
